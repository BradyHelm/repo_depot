...
async function addNewStory(evt) {
  ...
  // create a new Story instance and add it to the storyList
  const newStory = await storyList.addStory(currentUser, { title, url, author });

  // generate HTML for the new story and add it to the page
  const $newStory = generateStoryMarkup(newStory);
  $allStoriesList.prepend($newStory);

  // reset the form
  $('#create-story-form').trigger('reset');
  $('#create-story-modal').modal('hide');
}

// attach the event listener to the form submission
$('#create-story-form').on('submit', addNewStory);
